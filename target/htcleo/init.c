...
void target_init(void)
{
	...
	ptable_add( &flash_vptable, PTN_DEV_INF, flash_start_blk++, 1, 0, TYPE_APPS_PARTITION, PERM_WRITEABLE );
	...
	ptable_add( &flash_vptable, PTN_TASK_29, flash_start_blk, flash_size_blk, 0, TYPE_APPS_PARTITION, PERM_WRITEABLE );

	flash_set_temp_ptable( &flash_vptable );
...
void reboot_device(unsigned reboot_reason)
{
	fbcon_fill_rect(0,0,800,480,0x0000);
