...
typedef struct {
	...
	uint16_t	fatlength;		/* Length of FAT in sectors */
	uint16_t	fat_sect;		/* Starting sector of the FAT */
	uint16_t	rootdir_sect;	/* Start sector of root directory */
...
typedef long(file_read_func)(const char *filename, void *buffer, unsigned long maxsize);
...
int file_cd(const char *path); /* Currently this doesn't check if the dir exists or is valid... */
